...
- `src-tauri/src/policy.rs` gates every updater check. The updater plugin is built with `default_version_comparator`, which asks `PolicyStore::allows_release`. `useUpdater.hook.ts` calls the `check_policy` command (fetch, verify, decide) before each `check()`, 5 s after launch, every 15 minutes and on `system-wake`; a manual check skips the staged-rollout bucket for two minutes. The decision is kept in the updater slice (`policy`).
...
- Required updates (`update: "required"` and the found version is the target) show `UpdateRequired.dialogue.tsx`, which cannot be dismissed. The update downloads at once and installs only after dictation has been quiet for 30 s (`waitUntilIdle` in `src/lib/update-policy.util.ts`), then the app relaunches. Each install outcome is stored through `record_update_attempt`. `config.banner` shows `PolicyBanner`; `config.cloud_stt_enabled = false` stops cloud transcription in `useTranscription.hook.ts` and in `transcribe_buffer_cloud`.
