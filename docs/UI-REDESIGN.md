...
- Processing: on-device/cloud usage and correction rates live in a collapsed disclosure under Speech engine. Overview focuses on the user's words and activity.
...
The toolbar and sidebar establish the window frame. Content has a shared maximum width, with a narrower reading width for settings and utilities. Container queries respond to the actual page width, including when the sidebar is hidden. Pages remain scrollable at the existing 640 × 480 minimum; wide data tables can scroll within their own section.
...
| Sound strokes and sidebar/setup pattern | `src/components/shared/BrandMark.component.tsx` and `src/styles/brand.css` |
