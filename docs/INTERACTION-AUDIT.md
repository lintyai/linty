...
Native disclosure interpolation and discrete top-layer transitions are progressive enhancements. Older WebKit versions fall back to immediate visibility changes while keeping the controls functional. Sidebar visibility, responsive breakpoints, and opening the History reading pane still make their intended layout change; those dimensions are deliberately not tweened through repeated text reflow. Scroll memory is bounded and session-only and does not preserve every page's form state or selected filter across unmounts.
